...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "rule-application-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name rule-application-service apps/rule-application-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app rule-application-service:1.0-SNAPSHOT
    oc expose svc/rule-application-service
else

	echo "Launching the application locally..."
	pattern="rule-application-service"
	files=( $pattern )
	cd ${files[0]}
