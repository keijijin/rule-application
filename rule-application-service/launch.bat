...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "rule-application-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name rule-application-service apps/rule-application-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app rule-application-service:1.0-SNAPSHOT
		call oc expose svc/rule-application-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd rule-application-service
		cd target
